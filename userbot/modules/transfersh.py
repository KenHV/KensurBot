...
class TransferShMod(event.upload):
    """Upload to and from transfer.sh"""
    def __init__(self):
        self.message = event.uploadConfig("UPLOAD_URL", "https://transfer.sh/{}", "Url to PUT to")
        self.name = _("transfer.sh support")

@register(pattern=r"^.upsh(?: |$)(.*)", outgoing=True)
async def uploadshcmd(event):
        ...
        r = await event.run_sync(requests.put, self.config["UPLOAD_URL"].format(msg.file.name),
